 ...
 * @author jj
 */
public class ImportTreeStructureData extends ConsoleRunnable {
    ...
    private void importData(File file, int cancerStudyId) throws IOException, DaoException {
        ...
        try {
	        ...
	        long internalId = DaoTreeStructure.getLargestId();
          ...
          //       if (sample == null ) {
    	     //        ImportDataUtil.addPatients(new String[] { barCode }, cancerStudy);
                ...
                String descendentClone = strs[2].trim();
	            ...
	            if (p == null) {
	                assert StableIdUtil.isNormal(patientId);
	                entriesSkipped++;
	                continue;
	            }
	            TreeStructure tree = new TreeStructure(p.getInternalId(), cancerStudyId, ancestorClone, descendentClone);
	            tree.setInternalId(++internalId);
	            DaoTreeStructure.addTreeStructure(tree);
	        }
    ...
    public void run() {
        try {
		    String description = "Import 'phylogenetic_tree_structure' files";
		    ...
		    if (treeDataExistsForCancerStudy(cancerStudy)) {
			     throw new IllegalArgumentException("Phylogenetic tree data for cancer study " + cancerStudy.getCancerStudyStableId() + " has already been imported: " + dataFile);
		    }
		
		    //importTreeStructureFileMetadata(cancerStudy, properties);
		    importTreeStructureFileData(cancerStudy, dataFile);
    ...
    private static boolean treeDataExistsForCancerStudy(CancerStudy cancerStudy) throws DaoException {
        return (DaoTreeStructure.treeDataExistForCancerStudy(cancerStudy.getInternalId()));
    }

    // private static void importTreeStructureFileMetadata(CancerStudy cancerStudy, Properties properties) throws DaoException {
    ...
    private void importTreeStructureFileData(CancerStudy cancerStudy, String dataFilename) throws IOException, DaoException {
     ...
     */
    public ImportTreeStructureData(String[] args) {
        super(args);
    }
    ...
    public static void main(String[] args) {
        ConsoleRunnable runner = new ImportTreeStructureData(args);
        runner.runInConsole();
    }
}
